# This code derives from `https://github.com/zelloptt/zello-channel-api/blob/master/examples/py/opus_file_stream.py`


class OpusBinaryStream:
    # FIXME: Replace with `io.BytesIO`.
    ...
    def read(self, s: int):
        offset = self.offset
        self.offset += s
        return self.binary_data[offset:offset+s]


class OpusStream:
    # https://tools.ietf.org/html/rfc7845
    # https://tools.ietf.org/html/rfc3533
    def __init__(self, filename_or_content):
        if (type(filename_or_content) == str):
            filename = filename_or_content
            self.opusfile = open(filename, "rb")
            if not self.opusfile:
                raise NameError(f'Failed opening {filename}')

        elif (type(filename_or_content) == bytes):
            self.opusfile = OpusBinaryStream(filename_or_content)

        else:
            raise Exception('ILLEGAL OPUS FILE STREAM INPUT')
